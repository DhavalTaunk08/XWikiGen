...
class Dataset1(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'input_ids': input_ids.squeeze(), 'attention_mask': attention_mask.squeeze(), 'labels': labels.squeeze(), 'src_lang': src_lang, 'tgt_lang': tgt_lang, 'domain':domain}    
...
class Summarizer(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        return {'test_loss': loss, 'input_text': input_text, 'pred_text': pred_text, 'ref_text': ref_text, 'src_lang': src_lang, 'tgt_lang': tgt_lang, 'domain':domain}
    ...
    def test_epoch_end(self, outputs):
        ...
        df_to_write['src_lang'] = src_langs
        df_to_write['tgt_lang'] = tgt_langs
        ...
        logger.log_text(f'{method}_{t}_{filenum}_predictions', dataframe=df_to_write)
...
if __name__ == "__main__":
    ...
    method = ckpt_path_1.split('_')[0] + '_' + ckpt_path_1.split('_')[1] + '_' + ckpt_path_1.split('_')[2]
    ...
    filenum = test_path.split('/')[-1].split('_')[-1].split('.')[0]
    ...
    logger=WandbLogger(name='inference_' + method +  '_' + model_name, save_dir='./', project='multilingual multidomain evaluation', log_model=False)
    trainer = pl.Trainer(gpus=1, logger=logger, devices='auto')
