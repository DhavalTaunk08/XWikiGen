...
class CustomDataset(object):
    ...
    def __iter__(self) -> Iterator[Document]:
        for doc in self.docs:
            ...
            yield Document(sections=sections, reference=reference,page_title=doc.page_title,section_title=doc.section_names[0],content=doc.content)
    ...
    def __getitem__(self, i):
        if isinstance(i, int):
            ...
            sections = self._get_sections(doc)
            reference = self._get_reference(doc)
            return Document(sections=sections, reference=reference, page_title=doc.page_title, section_title=doc.section_names[0],content=doc.content)
        elif isinstance(i, slice):
            docs = self.docs[i]
            sections = [self._get_sections(doc) for doc in docs]
            references = [self._get_reference(doc) for doc in docs]
            return [Document(sections=s, reference=r,page_title=docs.page_title, section_title=docs.section_names[0],content=docs.content) for s,r in zip(sections, references)]
